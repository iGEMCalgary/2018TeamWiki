...
class Summarizer:
	...
	def summarizeText(self, txt):
		...
		top_n_scored = sorted(scoredSentences, key=lambda s: s[1])[-self.TOP_SENTENCES:]
		top_n_scored = sorted(top_n_scored, key=lambda s: s[0])
		
		meanSummary = ''
		
		for (idx, score) in mean_scored:
			meanSummary += ' ' + normalizedSentences[idx] [:1].capitalize() + normalizedSentences[idx][1:]
			
		topNSummary= ''
		for (idx, score) in top_n_scored:
			topNSummary += ' ' + normalizedSentences[idx][:1].capitalize() + normalizedSentences[idx][1:]
