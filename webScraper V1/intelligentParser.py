...
## Removes HTML, Script, and CSS tags from text ##
...
class Summarizer:
	...
	def summarizeText(self, txt):
		...
		for (idx, score) in mean_scored:
			summary += ' ' + normalizedSentences[idx] [:1].capitalize() + normalizedSentences[idx][1:]
		return summary

	## HTML, script, and css tag stripper driver ##
	...
	def scoreSentences(self, sentences, important_words):
		...
		for s in [nltk.tokenize.word_tokenize(s) for s in sentences]:
			...
			# Some sentences may not contain any important words at all.
